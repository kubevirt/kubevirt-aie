...
// mapped by reading each GPU's host NUMA node from sysfs and discovering its
// contiguous GI node range.
func buildGuestToHostMapping(domain *api.DomainSpec) (map[int]int, error) {
	...
	for _, hostDev := range domain.Devices.HostDevices {
		...
		primaryNode, giNodes, err := discoverGPUGINodes(bdf)
		if err != nil {
			log.Log.Reason(err).Warningf("Failed to discover GI nodes for device %s, skipping distance mapping", bdf)
			continue
		}

		hostGIRange := append([]int{primaryNode}, giNodes...)
		guestRange := guestEnd - guestStart + 1
		hostRange := len(hostGIRange)
		...
		for j := range count {
			mapping[guestStart+j] = hostGIRange[j]
		}
	}
...
// discoverGPUGINodes reads a GPU's primary NUMA node from sysfs and finds the
// contiguous GI nodes following it. Returns (primaryNode, extraGINodes, error).
func discoverGPUGINodes(bdf string) (int, []int, error) {
	numaPath := filepath.Join(SysfsPCIBasePath, bdf, "numa_node")
	data, err := os.ReadFile(numaPath)
	if err != nil {
		return -1, nil, fmt.Errorf("failed to read numa_node for %s: %w", bdf, err)
	}
	primaryNode, err := strconv.Atoi(strings.TrimSpace(string(data)))
	if err != nil {
		return -1, nil, err
	}
	if primaryNode < 0 {
		return -1, nil, fmt.Errorf("invalid NUMA node %d for device %s", primaryNode, bdf)
	}

	var giNodes []int
	candidate := primaryNode + 1
	for range maxGINodesPerGPU {
		if !isMemoryLessNoCPUNode(candidate) {
			break
		}
		giNodes = append(giNodes, candidate)
		candidate++
	}

	return primaryNode, giNodes, nil
}
