	...
	if device.ACPI != nil && device.ACPI.NodeSet == "tofill" {
		...
		topology.iommuDev = &api.IOMMUDevice{
			...
			Driver: &api.IOMMUDriver{
				...
				Oas:    fmt.Sprintf("%d", a.iommuPCI.OAS), // Output Address Size from CPU
				Ril:    "off",                             // Range-based Invalidation
				Accel:  "on",                              // Acceleration enabled
			},
		}

		// Set SubStream ID size if PASID is supported
		if bdf.PASIDSupported {
			topology.iommuDev.Driver.SSIDSize = fmt.Sprintf("%d", bdf.SSIDSize)
		}
