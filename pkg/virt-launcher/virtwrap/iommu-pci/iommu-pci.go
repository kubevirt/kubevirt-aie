...
type IommuPCI struct {
	...
	SMMUEnabled bool
	// OAS is the Output Address Size in bits supported by the CPU
	OAS int
...
type BDF struct {
	...
	// IOMMUFDEnabled indicates if IOMMUFD is enabled
	IOMMUFDEnabled bool
...
func NewIommuPCI(arch string) *IommuPCI {
	...
	if arch != "arm64" {
		log.Log.V(6).Info("Skipping SMMUv3 check for non-arm64 architecture")
		return &IommuPCI{}
	}

	// Step 1: Check if SMMUv3 (ARM IOMMU) is enabled on the system
	smmuEnabled, err := isSMMUv3Enabled()
	if err != nil {
		log.Log.Errorf("error checking SMMUv3: %v", err)
	}

	// Step 2: Determine the Output Address Size supported by the CPU
	// This determines the maximum physical address width for DMA operations
	oas, err := getOASFromCPU()
	if err != nil {
		log.Log.Errorf("Error getting OAS(defaulting to %d): %v", defaultOAS, err)
		oas = defaultOAS
	}
	log.Log.V(6).Infof("smmuv3: %t", smmuEnabled)
	log.Log.V(6).Infof("OAS: %d", oas)
...
//
// This method uses a hybrid strategy that tries modern IOMMUFD first,
// then falls back to legacy VFIO if necessary. It extracts:
...
func (bdf *BDF) ParseConfigHybrid() (*BDF, error) {
	iommufdEnabled, atsSupported, atsEnabled, pasidSupported, ssidSize, err := parseConfigHybrid(bdf.ID)
	if err != nil {
		log.Log.Errorf("Failed to process %s: %v", bdf.ID, err)
		return bdf, err
	}
	log.Log.V(6).Infof("ATS supported: %v, enabled: %v\n", atsSupported, atsEnabled)
	log.Log.V(6).Infof("PASID supported: %v, ssidSize: %d\n", pasidSupported, ssidSize)

	bdf.IOMMUFDEnabled = iommufdEnabled
